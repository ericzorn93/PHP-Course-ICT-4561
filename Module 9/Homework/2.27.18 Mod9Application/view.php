...
if ($db_connect) {
    ...
    $sql = "INSERT INTO imagedata (path, title, defined_title) VALUES ('{$directory_file_name}', '{$base_file_name}', '{$_POST['defined_image_title']}')";
    ...
    if (empty($upload_file)) {
        ...
        echo "<h1>View is Refreshing in 8 seconds...</h1>";
    }
}
...
<div class="container">
    ...
    while($image = mysqli_fetch_assoc($result)) {
        ...
        if (isset($_POST['delete_photo'])) {
            // Delete Query
            $sql_delete_second = "DELETE FROM `imagedata` WHERE `imagedata`.`id` = '{$image['id']}' AND `imagedata`.`title` = '{$image['title']}'";
            if ($db_connect->query($sql_delete_second) === TRUE) {
                // Delete Record from DB and from Uploads Folder
                unlink("uploads/{$image['title']}");
